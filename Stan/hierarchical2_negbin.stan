...
model {
  // HPP is gamma distribution
  m ~ gamma(shape, lambda0); 
  
  // Prior on theta is gamma(mean = 1, disp = alpha)
  theta ~ gamma(alpha, alpha);
  alpha ~ normal(0, 40);
  
  // Add on conjugate prior
  target += poisson_glm_conjprior(beta, X, m, lambda);
  
  // subtract log nc of conj prior via laplace
  target += -poisson_lognc_irls(m, X, lambda, start, tol, maxit);
  
  // Likelihood
  y     ~ poisson_log( log(theta) + X*beta );
}
