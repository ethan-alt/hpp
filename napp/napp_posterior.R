...
## source norm power prior R function
stanfile.path <- '~/Projects/Paper3/extensive_sims/napp'
setwd(stanfile.path)
source('glm_napp_R.R')
...
for ( i in 1:each.cl ) {
  ...
  smpl = tryCatch(
    glm_napp(
      formula, poisson('log'), data = data, histdata = histdata, chains = chains, iter_warmup = warmup, iter_sampling = nsmpl, thin = thin,
